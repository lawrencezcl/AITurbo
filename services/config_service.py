...
class ConfigService:
    ...
    def _get_config_from_env(self) -> Dict[str, Any]:
        ...
        if wechat_appid:
            config['wechat_appid'] = wechat_appid
            
        wechat_appsecret = os.getenv('WECHAT_APPSECRET')
        if wechat_appsecret:
            config['wechat_appsecret'] = wechat_appsecret
        ...
        if dashscope_api_key:
            config['dashscope_api_key'] = dashscope_api_key
        ...
        if content_source_url:
            config['content_source_url'] = content_source_url
    ...
    def get_database_config(self) -> Dict[str, str]:
        ...
        return {
            ...
            'database': config.get('db_name', 'aiturbo')
        }
        ...
        def monitor():
            while True:
                try:
                    ...
                    now = int(time.time())
                    remain = expire_time - now if expire_time else 0
                    try:
